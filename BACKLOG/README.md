...
| EPIC | Description |
|------|-------------|
| [EPIC-18 — Analytics CNIL-friendly (Matomo)](EPIC-18-analytics-matomo/README.md) | Test parallele Matomo (instance perso de Maxime) vs GA, puis arbitrage. Permet de tracker 100 % des visiteurs sans bandeau cookie en mode "consentless tracking" CNIL. Phase 1 livree 2026-05-11, **Matomo tourne en prod en parallele de GA depuis**. Reste : **bug prod** iframe opt-out bloquee par la CSP (`frame-src`), recette PO de la config privacy, puis **arbitrage GA vs Matomo** (la fenetre de test de 2-4 semaines est ecoulee). |
